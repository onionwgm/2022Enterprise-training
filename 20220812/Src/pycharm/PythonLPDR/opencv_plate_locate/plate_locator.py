...
def get_candidate_plates_by_sobel(plate_image):
    ...
    for i in np.arange(len(contours)):
        ...
        if util.verify_plate_sizes(contour, plate_image):
            # 完成旋转
            output_image = util.rotate_plate_image(contour, plate_image)
            # 统一尺寸
            uniformed_iamge = util.unify_plate_image(output_image)
            # 追加到车牌区域的候选区域列表中
            candidate_plates.append(uniformed_iamge)
...
# 参数：车牌（含有车牌的车辆）图片
# 返回值：所有可能的车牌候选区域 = list
def get_candidate_plates_by_hsv(plate_image):
    # 1. 对含有车牌的车辆图片进行预处理(hsv)
    preprocess_image = util.preprocess_plate_image_by_hsv(plate_image)
    # 2. 提取所有的等值线|车牌轮廓(可能)的区域
    contours, _ = cv.findContours(preprocess_image, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    # 3. 判断并获取所有的车牌区域的候选区域列表
    candidate_plates = []
